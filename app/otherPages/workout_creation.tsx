   ...
   <SafeAreaProvider>
    <SafeAreaView className='bg-dark-100' style={{flex: 1, alignItems: "center"}}> 
     ...
     <View className="flex-1 items-center justify-center h-auto w-[80%]">
      <TextInput placeholder='Enter Workout Name' placeholderTextColor={'darkgrey'}
                value={name} onChangeText={setName}
                style={{paddingVertical:30}} className='text-center text-light-100 bg-white rounded-md px-4 w-full mb-4'/>

      {name.length>0 && (<View className=''>
                         <CustomButton buttonText='Create Workout' 
                                      onPress={async ()=>{const resultId = await addWorkout(name);
                                                          if(resultId==-2) return;
