...
export const SetView = ({set_number,previous_weight}:{set_number:number;previous_weight?:number}) => {
  ...
  return (
      ...
      <View className='flex-row justify-around mt-2 items-center' >
        <Text className='text-light-100 text-xl mb-1'>Set {set_number}</Text>
        <View className='flex-row gap-5 justify-end'>
          <TextInput placeholder={(weight).toString()+' kg'}
...
export const ExerciseView = ({name,sets}:{name:string,sets:Array<number>}) => {
  ...
  return (
    <View className='mb-5 px-7'>
      ...
      <View className='justify-center'>
        <FlatList data={sets}
                renderItem={(item)=>(<SetView set_number={item.item}/>)}
